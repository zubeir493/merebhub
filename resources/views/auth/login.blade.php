...
@section('content')
    <div class="mx-auto max-w-md px-5 py-16">
        <h1 class="text-3xl font-extrabold tracking-[-0.035em] text-zinc-950">{{ $title ?? 'Welcome back' }}</h1>
        <p class="mt-2 text-sm text-zinc-600">{{ $subtitle ?? 'Sign in to access your licenses and downloads.' }}</p>
        <form method="POST" action="{{ route('login', request()->query()) }}" class="mt-8">
            @csrf
        ...
        <p class="mt-6 text-center text-sm text-zinc-600">New to MerebHub? <a href="{{ route('register', request()->query()) }}" class="font-extrabold text-teal-700">Create an account</a></p>
    </div>
@endsection
