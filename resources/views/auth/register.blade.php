...
@section('content')
    <div class="mx-auto max-w-md px-5 py-16">
        ...
        <form method="POST" action="{{ route('register', request()->query()) }}" class="mt-8">
            @csrf
        ...
        <p class="mt-6 text-center text-sm text-zinc-600">Already registered? <a href="{{ route('login', request()->query()) }}" class="font-extrabold text-teal-700">Sign in</a></p>
    </div>
@endsection
