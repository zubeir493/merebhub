...
@section('account-content')
    <header class="account-page-header">
        <h1 class="account-page-title">Security &amp; sessions</h1>
        <p class="account-page-description">Review recent signed-in devices. Signing out a device removes its Laravel session from the configured session store.</p>
    </header>

    <section class="account-surface overflow-hidden">
        @forelse ($sessions as $session)
            ...
            <article class="flex flex-col gap-4 border-b border-zinc-100 px-5 py-5 last:border-0 sm:flex-row sm:items-center sm:justify-between sm:px-6">
                <div class="flex items-start gap-4">
                    <span class="grid size-11 shrink-0 place-items-center rounded-md {{ $isCurrent ? 'bg-teal-50 text-teal-700' : 'bg-zinc-100 text-zinc-600' }}">
                ...
                @else
                    <form method="POST" action="{{ route('account.security.sessions.revoke', $session->public_id) }}">
                        @csrf
                        <button type="submit" class="min-h-11 rounded-md border border-rose-200 px-4 py-2 text-sm font-bold text-rose-700 hover:bg-rose-50">Sign out device</button>
                    </form>
                @endif
            </article>
        @empty
