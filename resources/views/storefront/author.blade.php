...
@section('content')
    <div class="mx-auto max-w-[1500px] px-5 py-10 lg:px-8">
        <section class="overflow-hidden rounded-2xl border border-zinc-200 bg-white">
            <div class="h-40 bg-gradient-to-br from-zinc-900 via-teal-900 to-teal-600 sm:h-56">
            ...
            <div class="px-5 pb-7 sm:px-8">
                <div class="-mt-12 flex flex-col gap-5 sm:flex-row sm:items-end">
                    @if ($author->avatarUrl())
                        <img src="{{ $author->avatarUrl() }}" alt="{{ $author->name }}" class="size-24 rounded-2xl border-4 border-white bg-white object-cover">
                    @else
                        <span class="grid size-24 place-items-center rounded-2xl border-4 border-white bg-teal-50 text-teal-700"><x-heroicon-o-building-office-2 class="size-10" /></span>
                    @endif
                    <div class="flex-1">
                        <div class="flex flex-wrap items-center gap-2">
                            <h1 class="text-3xl font-extrabold text-zinc-950">{{ $author->name }}</h1>
                        ...
                        @if ($author->tagline)<p class="mt-1 text-zinc-600">{{ $author->tagline }}</p>@endif
                    </div>
                    <div class="flex gap-5 text-sm font-bold text-zinc-600">
                ...
                <div class="mt-6 grid gap-6 border-t border-zinc-100 pt-6 lg:grid-cols-[minmax(0,2fr)_minmax(16rem,1fr)]">
                    <p class="leading-7 text-zinc-600">{{ $author->bio }}</p>
