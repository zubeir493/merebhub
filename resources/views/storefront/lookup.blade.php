...
@section('content')
    <div class="mx-auto max-w-xl px-5 py-16">
        <div class="text-center">
            ...
            <p class="mt-3 text-sm leading-6 text-zinc-600">Use the email from checkout and your MerebHub order reference.</p>
        </div>
        <form method="post" action="{{ route('orders.lookup.result') }}" class="mt-8 rounded-lg border border-zinc-200 bg-zinc-50 p-6">
            ...
            <input name="order_reference" value="{{ old('order_reference') }}" class="form-input" required>
            @error('order_reference')<p class="form-error">{{ $message }}</p>@enderror
