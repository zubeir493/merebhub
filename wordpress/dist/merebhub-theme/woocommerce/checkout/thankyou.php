...
<section class="mh-thankyou">
    <?php if ($order) : ?>
        <?php if ($order->has_status('failed')) : ?>
            <span class="dashicons dashicons-warning"></span><h1><?php esc_html_e('Payment was not completed', 'merebhub'); ?></h1><p><?php esc_html_e('Please try your payment again or contact support.', 'merebhub'); ?></p>
        ...
        <?php else : ?>
            <span class="dashicons dashicons-yes-alt"></span><h1><?php esc_html_e('Thank you for your purchase', 'merebhub'); ?></h1><p><?php esc_html_e('Your order is confirmed. License details appear below and have also been sent by email.', 'merebhub'); ?></p>
        ...
        <?php endif; ?>
    <?php else : ?>
        <span class="dashicons dashicons-yes-alt"></span><h1><?php esc_html_e('Thank you', 'merebhub'); ?></h1>
    <?php endif; ?>
</section>
<?php if ($order) { do_action('woocommerce_thankyou_'.$order->get_payment_method(), $order->get_id()); do_action('woocommerce_thankyou', $order->get_id()); } ?>
