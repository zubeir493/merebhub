...
$top = merebhub_product_query([
    'meta_key' => 'merebhub_weekly_sales',
...
<main id="primary">
    <section class="mh-hero">
        ...
        <div class="mh-shell mh-hero__content">
            <div>
                ...
                <a class="button mh-button" href="<?php echo esc_url(wc_get_page_permalink('shop')); ?>"><?php esc_html_e('Explore software', 'merebhub'); ?><?php echo merebhub_icon('arrow-right'); ?></a>
            </div>
        </div>
    </section>
    ...
    <div class="mh-shell mh-home-content">
            ...
            while ($top->have_posts()) {
                ...
                $product = wc_get_product(get_the_ID()); ?>
                        <a href="<?php the_permalink(); ?>" class="mh-ranked-product">
                            ...
                            <span><b><?php the_title(); ?></b><small><?php echo merebhub_icon('star', 'mh-star'); ?> <?php echo esc_html($product->get_average_rating() ?: 'New'); ?> &middot; <?php echo esc_html(implode(', ', wp_get_post_terms(get_the_ID(), 'product_cat', ['fields' => 'names']))); ?></small><em><?php echo wp_kses_post($product->get_price_html()); ?></em></span>
                        </a>
                    <?php } wp_reset_postdata(); ?>
                </div>
            </section>
        ...
        <?php if ($featured->have_posts()) { ?>
            <section class="mh-home-section">
                <header class="mh-section-heading"><h2><?php esc_html_e('Featured software', 'merebhub'); ?></h2><a href="<?php echo esc_url(wc_get_page_permalink('shop')); ?>"><?php esc_html_e('See all', 'merebhub'); ?></a></header>
                <?php merebhub_render_product_grid($featured); ?>
            </section>
        <?php } ?>

        <?php if (! is_wp_error($section_categories)) {
